...
export const SFX = {
  ...
  coin()       { beep(988, 0.04); beep(1319, 0.1, 'square', 0.3, 0.04); },
  powerup()    { [523,659,784,1047].forEach((f,i) => beep(f, 0.1, 'square', 0.25, i*0.08)); },
  stomp()      { beep(200, 0.04, 'square', 0.4); beep(100, 0.08, 'square', 0.3, 0.04); },
  hurt()       { beep(440, 0.05, 'sawtooth', 0.4); beep(220, 0.2, 'sawtooth', 0.3, 0.05); },
  death()      { [440,400,350,300,250,200,150].forEach((f,i) => beep(f, 0.12, 'sawtooth', 0.3, i*0.07)); },
  flagpole()   { [523,659,784,1047,1319].forEach((f,i) => beep(f, 0.15, 'square', 0.3, i*0.1)); },
  levelwin()   { [784,988,1175,988,784,659,784].forEach((f,i) => beep(f, 0.18, 'square', 0.3, i*0.12)); },
  ...
  break()      { beep(120, 0.05, 'noise', 0.4); beep(80, 0.1, 'sawtooth', 0.3, 0.05); },
  star_collect(){ [784,988,1175,1319,1568].forEach((f,i) => beep(f, 0.08, 'square', 0.3, i*0.05)); },
  enemy_kick() { beep(300, 0.05, 'square', 0.35); beep(150, 0.08, 'square', 0.25, 0.05); },
  checkpoint() { beep(659, 0.1); beep(784, 0.15, 'square', 0.3, 0.1); },
  extra_life() { [523,659,784,1047,784,659,523,784,1047,1319].forEach((f,i) => beep(f, 0.1, 'square', 0.25, i*0.09)); },
...
const TRACKS = {
  ...
  starmode: {
    ...
    bass: [
      [196,0.5],[196,0.5],[196,0.5],[196,0.5],
      [165,0.5],[165,0.5],[165,0.5],[165,0.5],
